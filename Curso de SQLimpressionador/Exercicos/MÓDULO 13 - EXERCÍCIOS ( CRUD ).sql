...
DECLARE @proximo_id_cliente INT = (SELECT COUNT(ID_Cliente) FROM dCliente) + 1
INSERT INTO dCliente(ID_Cliente,Nome_Cliente,Data_de_Nascimento)
VALUES
	(@proximo_id_cliente,'Arthur Ribas','2011-11-10')

DECLARE @proximo_id_gerente INT = (SELECT COUNT(ID_Gerente) FROM dGerente) + 1
INSERT INTO dGerente(ID_Gerente,Nome_Gerente,Data_de_Contratacao,Salario)
VALUES
	(@proximo_id_gerente,'BRUNA RHAYSSA','2021-21-1',15000)

DECLARE @proximo_id_contrato INT = (SELECT COUNT(ID_Contrato) FROM fContratos) + 1
INSERT INTO fContratos(ID_Contrato,Data_de_Assinatura,ID_Cliente,ID_Gerente,Valor_do_Contrato)
VALUES
	(@proximo_id_contrato,'2021-15-12',(SELECT COUNT(ID_Cliente) FROM dCliente) + 2,(SELECT COUNT(ID_Gerente) FROM dGerente),5000)
...
WHERE ID_Contrato = 4
 ...
 WHERE ID_Contrato = (SELECT COUNT(ID_Contrato) FROM fContratos)
